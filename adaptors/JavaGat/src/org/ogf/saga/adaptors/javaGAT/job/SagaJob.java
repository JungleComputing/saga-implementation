...
public final class SagaJob extends org.ogf.saga.impl.job.JobImpl implements
    ...
    public SagaJob(JobServiceAdaptor service,
        ...
        try {
            int hostCount = Integer
                    .parseInt(getV(JobDescriptionImpl.NUMBEROFPROCESSES))
                    / Integer
                            .parseInt(getV(JobDescriptionImpl.PROCESSESPERHOST));
        ...
        try {
            ...
            for (String e : env) {
                ...
                if (index == -1) {
                    ...
                    environment.put(e.substring(0, index), e
                            .substring(index + 1));
                }
            }
        ...
        if (transfers != null) {
            for (int i = 0; i < transfers.length; i++) {
                ...
                if (parts.length == 1) {
                    ...
                    throw new NotImplementedException(
                            "PreStage append is not supported", this);
                }
                ...
                if (parts.length == 1) {
                    ...
                    if (parts.length == 1) {
                        throw new BadParameterException(
                                "Unrecognized FileTransfer part: "
                                        + transfers[i], this);
                    }
                }
                ...
                if (!prestage) {
                    if (stdout != null && !stdout.isAbsolute()
                            && s2.equals(stdout)) {
                ...
                if (prestage) {
                    if (logger.isDebugEnabled()) {
                        logger.debug("Add prestage: src = " + s1 + ", dst = "
                                + s2);
                    }
                    sd.addPreStagedFile(f1, f2);
                } else {
                    if (logger.isDebugEnabled()) {
                        logger.debug("Add poststage: dst = " + s1 + ", src = "
                                + s2);
                    }
    ...
    private File createFile(URI uri) throws BadParameterException {
        try {
            ...
            throw new BadParameterException("Could not create GAT File for "
                    + uri, e, this);
        }
    }
    ...
    public void checkpoint() throws NotImplementedException,
            ...
            TimeoutException, NoSuccessException {
        throw new NotImplementedException(
                "checkpoint() not implemented: JavaGAT does not support it",
                this);
    }
    ...
    public void resume() throws NotImplementedException,
        ...
        if (s != State.SUSPENDED) {
            throw new IncorrectStateException(
                    "resume() called when job state was " + s, this);
        }
        try {
            gatJob.resume();
        } catch (GATInvocationException e) {
    ...
    public void suspend() throws NotImplementedException,
        ...
        if (s != State.RUNNING) {
            throw new IncorrectStateException(
                    "suspend() called when job state was " + s, this);
        }
        try {
            gatJob.hold();
        } catch (GATInvocationException e) {
